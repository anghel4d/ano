...
// The palette, xterm-256. Cell fg/bg 0 mean the canvas defaults.
...
// The terminal state: grid row-major grid[y*cols+x]; out is the frame
...
// Event model. C-style discriminants: the decode tree ports 1:1.
...
impl Term {
    ...
    // (contents discarded; the next frame_clear repaints).
    ...
    // are skipped but still consume width; glyph bytes clamp at 7.
    ...
    // the top rule as ╴title╶ from x+2, maxw w-6. No-op when w<2 or h<2.
    ...
    // C_FRAME off. Integer math exact.
    ...
    // "\x1b[<y+1>;<x+1>H\x1b[<shape> q\x1b[?25h". No diffing.
...
// The full decode tree: outer sys::rbyte(100), continuation bytes
