...
1. **Land REGFIX.md.** The case contract (names fold, values never), `as` — name alias and derived tag — one-resolver roles, the s56/s57 witnesses, then `--dump`. After it lands: the demo/registry revision — only where a demo encodes wrong semantics, declared per-demo, executed by Opus 4.8 subagents under Fable supervision, never inline. (Added 2026-07-08.)

2. **Consider shipping proven-affine compilation.** Every statement is affine and alias-free by grammar, so the polyhedral arsenal (fusion, tiling, vectorization, parallelization) applies to all statements with no legality analysis. A backend that proves the schedule and emits raw column instructions is the Groq/TPU bet scoped to ano's fragment; the same property gives the CUDA lowering for free. Grading and lineage: `ano-sky.md`, "The speed claim, graded". (Added 2026-07-05.)

## Closed

2026-07-08 — Four rulings, recorded where they execute across REGFIX.md's items and Docs: names always case-insensitive (defs included; a fold collision is an emit-time error), values always case-sensitive; one resolver for roles (`reg_role` falls back to `reg_find`); the emitter's contextual specials stay exact-lowercase and shadowable by entries; keywords exact, declared names fold. The demos' STABLE seal lifted. NEXT.md and NEXT-2.md executed and deleted; their record lives in PATCHES.md and git history (178172b, b8ea048).
