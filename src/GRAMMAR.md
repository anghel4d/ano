...
- `pres <col> <mask>` and `default <name> <v>` — presence and spawn default, declared after their entry.
- `rel <name> <n values>` — functional, -1 dangling. `srel <name> <fibers |-separated>` — set-valued, empty fibers legal. `inv <name> <rel>` — the inverse read, fibers computed at load.
