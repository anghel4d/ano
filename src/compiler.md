# compiler.md — anoc, the C oracle

Status: implemented and frozen behind Steel.

`anoc` compiles both surfaces to BQN and asserts pinned post-states. Steel is the Rust reference implementation. The C tree remains its differential oracle. Both use CBQN as the execution backend.
...
main.c — the driver. It reads directives, runs lex → parse → emit, appends BQN assertions, and invokes CBQN. `--tokens` and `--emit` expose the two intermediate forms. `--dump` writes the loaded registry through a staged rename. `--save` accepts post-state over the 0x1E child channel and commits only after a successful run. Kore uses that channel for REPL ticks.
