...
- `main.c` — driver: `anoc [--tokens] [--emit] [--run] [--dump <path>] [--rt <path>] [--registry <path-or-name>] file.ano`; `--!` directives name the registry and pin expectations, compiled into BQN assertions, so `--run`'s exit code is the differential-test verdict.
