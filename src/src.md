...
- `registry.c` — dummy-registry loader; the `.reg` file format is specified at `reg_load` in ano.h. A registry stands in for the host: columns, presence, relationships (functional, set-valued, inverse reads), aliases, bindings, and callables carrying their own BQN — the eventual host ECS replaces exactly this file.
...
- `dummy/*.reg` — per-demo fixtures reproducing each .bqn demo's initial world exactly.
