...
- `15-named-reducer.bqn` — `fold(threat)` as a registered associative reducer; no identity, so the empty scope fails the row (BQN's `¯∞` identity noted as a divergence).
...
- `s62-ever-any.bqn` — the `|\` latch (`∨` scan): has the fire reached each point along the route; trips on at the first true and stays on.
- `s63-still-all.bqn` — the `&\` latch (`∧` scan): the column intact up to here; trips off at the first false and stays off.
- `s64-reducer-spellings.bqn` — one registered reducer, three spellings: `threat/`, `fold(threat)`, `threat\` — the two folds pinned against one value, the scan as running peak.
