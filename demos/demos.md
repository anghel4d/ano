...
- `13-typed-registries/` — 133-137, the carrier refinements (Steel, 2026-07-12): `bool` enforced by the 0< retraction (133), `nat` — the naturals, floor at 0 with no underflow (134), floor to the integer grid and the 2^53 ceiling (135), `int` its signed twin, ⌊ toward −∞ (136), and the declared `range` bounds over the untyped double (137). Rest data seals at load (the negative battery pins the refusals), committed writes retract at the barrier, the TUI clamps and warns.
