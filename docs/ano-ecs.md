...
The first value refinements shipped with Steel (2026-07-12): `bool` — {0,1}, `nat` — the naturals to 2^53, `int` — their signed twin, and the `range <col> <lo> <hi>` rider over any numeric carrier. The .reg kind word is the descriptor bit; in the C-struct ABI it lands beside `merge` like every other tag. Enforcement is placed by boundary ownership: load seals rest data (out-of-domain refuses, the `unique` precedent), the scatter retracts every committed write onto the carrier set (bool by positive-is-true, nat/int by floor then clamp, range by clamp — once per barrier, however many writes merged), the editor repairs with a warning. A mask op on a bool column elides the retraction; spawn fill is in-domain by the load seal (a range excluding the declared default refuses), so appends never wrap.
