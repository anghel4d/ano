...
Ano's Greater/Lesser family is exactly q/kdb+'s convention over the carriers that Ano admits; this is not a claim of general q compatibility. `a | b` is OR on masks, pointwise maximum on numbers, and the pointwise greater rune on glyphs. `a & b` is AND on masks, pointwise minimum on numbers, and the pointwise lesser rune on glyphs. The glyph order is the code-point order. Their folds and scans are the corresponding reductions and scans. Ano admits same-carrier instances only: the ruling is that runtime expressions perform no implicit cross-carrier coercion, and every mixed-carrier operand refuses before lowering, uniformly across the operator families. Masks carry selection semantics — presence, validity, foundness — that a silent 0/1 lift would erase; the glyph order is symbolic, not arithmetic; and q's own two lift directions (`98 | "a"` is the char `"b"` while `"a" + 1` is an int) are licensed by chars-as-bytes, a representation fact Ano does not accept as a semantic license. Cross-carrier movement is an explicit registered conversion, whose surface belongs to the author-gated registry taxonomy of `todo/05`. Registry declarations remain strict and every committed write is checked against its destination carrier. The ruling is deliberately the conservative pole: a later per-operator coercion table would be a monotone extension — a refusal becoming a value breaks no program — while the reverse migration would break programs, so refusal is the only safe default to start from. See KX's definitions of [Greater](https://code.kx.com/q/ref/greater/), [Lesser](https://code.kx.com/q/ref/lesser/), [max](https://code.kx.com/q/ref/max/), and [min](https://code.kx.com/q/ref/min/).
...
The overlay runtime, the resolver framework, the host boundary, replay, the ruled `^cursor` cold default at the elided-subject site, and Kore's display are implemented in Steel and Kore; `todo/02-dynamic-alias-overlay.md` still owes the concrete host deictic resolvers, which ride with `99`'s host services, and the two alias demos quarantined behind `99`.
...
- Numeric exceptional values. Steel's present registry-load and save boundaries admit finite IEEE float64 values only. The empty-fold ruling says an identityless fold over an empty scope has no result row; it does not decide whether `num` admits `±∞` or NaN, whether Ano has an absence value, or whether admitted infinities become registered extrema identities. Those are separate decisions.
