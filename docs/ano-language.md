...
Dynamic aliases occupy an overlay namespace keyed by the bare spelling. `^Whiterun` reads the live `Whiterun` alias target, such as another column or an existing mask, when one exists and otherwise falls through to bare `Whiterun`. Installing, rebinding, or deleting the alias changes only `^Whiterun`; bare `Whiterun` remains the registered column or binding. The caret promises only that the referent may change between statement steps. Steel registers service-v1 resolvers for `^cursor`, `^observer`, `^selected`, and `^world`; each runs only against the statement-frozen world and host input. A cold effect subject desugars to the same `^cursor` request, overlay first and bare fallback second; a continuation still requires its saved antecedent.
...
The keyed hop. A `unique` column is declared injectivity — pairwise-distinct, checked at load — and injectivity is precisely the license a hop needs, since an injective column is invertible on its image: the keyed hop is `rel ; unique⁻¹`, one index-of against the key column with one found-guard. A relationship declares its key column by writing it before the name (`rel id mentor -1 0 0 3 …`, reading type-annotation-first: mentor is a rel over id), and an undeclared rel stays keyed to the row index — zero churn for every existing world. Not-found is dangling is dead: a stored ID whose target despawned fails the found-guard and clears the mask bit, the same left-join-null, so staleness never grows a fault path (the storage mechanism is ano-ecs §2's generational compare; the diagnostic surface, never the semantics, is the debug layer's dead-link report). `DEAD` deliberately names every non-sentinel stored target that fails the found-guard. Never-existed, despawned, and mistyped are not separate language states. The `-1` None sentinel stays silent. The inverse of a keyed rel is keyed automatically, and `unique` subsumes the id/keys name magic: it declares the key space. `unique` means exactly one thing: every element distinct, enforced at load and held across the run. Minting is spawn machinery, never a `unique` semantic, so several unique columns never compete for a mint; spawn fills each one fresh because the declared injectivity admits nothing else.
...
undef spread                                                                  -- withdrawn at a later barrier
```
...
The stage-4 rules stand inert until the data says otherwise. Once advancement is sealed, the mission schedule issues `undef stage3`; the explicit barrier edge keeps rule-set mutation out of the rule effect algebra and records the withdrawal in replay order.
...
Ano's runtime promotion is operator-specific, while registry storage remains strict. Arithmetic promotes masks and glyphs to numbers: mask bits become `0` or `1`, glyphs become Unicode code points, and the result is numeric. Greater/Lesser joins `mask < number < char`: the lower operand converts upward and the result has the joined carrier, so q's `98 | "a"` result `"b"` is admitted without making char arithmetic character-valued. Comparisons convert glyphs to code points, compare masks and numbers numerically, and return a mask; symbols admit only same-carrier equality and inequality. Folds and scans apply the same family rule to their one input carrier. No universal subtype relation is inferred from these conversions. A result crossing the effect border must inhabit the declared destination carrier, and every numeric publication refuses NaN before commit. See KX's definitions of [Greater](https://code.kx.com/q/ref/greater/), [Lesser](https://code.kx.com/q/ref/lesser/), [max](https://code.kx.com/q/ref/max/), and [min](https://code.kx.com/q/ref/min/).
...
A seeded fold and a fallible `traverse` live in `todo/06-seeded-fold-and-traverse.md`.
...
- An explicit binding such as `Player` or `Whiterun` has exactly one declared face and carrier. Entity bindings store stable keys and resolve them against the current key column, never against row offsets; mask, point, number, and vector bindings remain their own disjoint faces. A same-stem sigiled alias is a separate dynamic lookup and never changes that bare denotation.
...
The canonical registry extension is `.reg`. The non-spatial high-integrity declarations are `array`, typed `fn`, `service`, `enum`, and `ctor`; each carries an explicit nonzero 64-bit declaration ID and positive local version. Primitive registry carriers are closed, named carriers refer backward to an earlier enum or constructor, callable signatures and footprints are exact, raw BQN requires the trusted boundary, enum discriminants are stable serialized identities, and nominal values arise only through checked constructors. Legacy declarations remain loadable but acquire no typed authority by proximity.

A resident array line declares only a typed slot. A host attachment is separately sealed against carrier, domain, live entity keys or nominal refinement and receives a stamp containing the complete schema fingerprint, declaration ID, declaration version, and canonical name. Constructed nominal values carry the same stamp. Their canonical one-line sidecars are bit-exact and always revalidate on decode; `docs/ano-registry.md` owns the exact registry and sidecar syntax.

The registry also carries accumulator entries and algebraic witnesses. A reducer supplies a carrier-compatible step; identity, finish, associativity, and commutativity are independent optional capabilities. Identity licenses a value for the empty fold, associativity licenses unordered regrouping over a fixed traversal, and associativity plus commutativity license parallel/unordered execution that may discard traversal order. Exact left accumulation on a declared order needs only the compatible step. A merge operation owes the law that makes collision fibers deterministic. A locator owes its choice law; a support projector owes candidate, `Best`, and refusal laws under a stable semantic tie policy. These are local capabilities of operations on carriers. Ground entries are trusted executable facts. A sky law grants optimizer rewrite authority only through a machine-checked certificate whose proposition names the exact typed operation and schema identity; trust and property tests may admit or diagnose execution but never authorize a rewrite. `ano-sky.md` carries the certificate boundary.
...
### Schema identity and replacement

One Ano statement and one tick execute under one fixed schema `Σ`. Ordinary writes divide into value updates under existing column declarations, population changes such as spawn and despawn that preserve declaration identity, and placement or transform values whose declared types remain fixed; the last arm names the law but does not claim the spatial implementation reserved for task `99`. None of these mutates the registry grammar or lets a row observe a mid-statement schema.

A `Σ → Σ′` change is a host operation at a world barrier, never an Ano effect. Steel parses the candidate independently, requires a complete explicit declaration map, stages supported carrier conversions, seals relationship endpoints, migrates the live alias overlay, and mints a new `(version,fingerprint)`. The fingerprint covers every current declaration descriptor, spelling alias, role, and reap policy while deliberately excluding live row values and population. Explicit high-integrity IDs survive `preserve` and `rename`; legacy manifest IDs also survive supported `widen`; additions mint IDs; removals with live data require `discard`; aliases targeting a removed declaration require `unalias`. The pre-spatial widening relation remains exactly `bool → nat|int|num`, `nat → int|num`, and `int → num`. A high-integrity semantic change retains its stable ID but must increase its declaration version; regression or same-version change refuses. Every compiled plan and resolved or resident capability is schema-stamped and must be revalidated through the migration receipt or discarded.

The host-state boundary is deliberately outside both grammars: `<world>.reg.schema` stores legacy and explicit stable IDs with schema version, `<world>.reg.aliases` stores the session overlay in one canonical LF-terminated UTF-8 form published by create-new staged atomic replacement, and `<world>.reg.migrations` records deterministic events. A resolver alias stamps a declared service's stable ID and version; the distinct legacy-v1 contract cannot silently replace or become declared-v1. Resident arrays and constructed nominal values use separate canonical schema-stamped sidecars and never smuggle payload into declaration text. `kore migrate live.reg candidate.reg migration.map` publishes the registry bundle as one rollback-safe journaled transaction; receipt APIs then reseal external runtime capabilities under `Σ′` before their owner publishes them. The typed migration-extension trait is the only route by which task `99` may add lattice, habitat, placement, and spatial-service conversion.
...
Every validity or foundness guard that survives a speculative effect-value probe must be a closed predicate over durable world bindings. A probe-local staged temporary may not escape after its defining buffer is discarded; Steel refuses that plan before emission, so guard closure is an enforced compiler invariant rather than a lowering convention.
...
Steel implements the carrier overload and promotion table before lowering. Direct `|` and `&` join mask, number, and char upward; their one-operand folds, scans, and along forms retain the operand carrier. Arithmetic folds and scans promote mask bits and glyph code points to number, while `#` consumes selection presence rather than payload. `max/`, `max\`, `min/`, and `min\` resolve to the same descriptors as `|/`, `|\`, `&/`, and `&\` on the carrier at hand, so each bridge pair emits one program. The glyph extrema travel through code points because the backend's own extrema refuse characters; one declared step serves the dyad, fold, and scan, and the char order carries no identity, so an empty glyph scope yields no result row. `#\` and `avg\` are live prefix machines. The γ column-form takes operator folds, and `#/ rel'.Comp` counts the fiber's admitted elements rather than summing their payload. A named reducer over fibers (`threat/ livestock'.Weight`) is still refused, and so is every per-fiber scan, because a ragged per-fiber result has no representation yet.
...
cross dist Tower Creep                      -- a rank-2 product value; it retains product lineage and cannot scatter as an entity column (§20)
...
eval "Nord , Gold += 100"                  -- the complete quotation surface: one literal statement, re-lexed and parsed in place at compile time
def spread = Plot & p => +Planted          -- the naru hinge: same form, installed instead of performed, re-gathered once per tick (§11)
undef spread                               -- explicit named-rule retraction at a barrier (§11)
```
...
Three mechanisms wear the word alias and each has its own lifecycle. Spelling aliases are the registry's `as` and `ja` rows: accepted spellings mapped to canonical registry names, immutable with the registry, applied during ordinary bare resolution. The static alias mask is the registry `alias` directive: a stored mask value living in registry data like any other column. The dynamic overlay is what `^name` reads: host-managed session state persisted beside the registry in its `.aliases` sidecar, consulted only for the sigiled request, falling through to the bare binding when the stem is absent. The registry directive never seeds the overlay, so no declaration carries two lifetimes.

The overlay runtime, concrete deictic resolver table, cold `^cursor` default, host boundary, replay, migration revalidation, Kore display, and non-spatial demo rewrites are implemented in Steel and Kore. The two remaining quarantined alias units, `071` and `093`, also require the spatial contract in task `99`.

## Rulings, Open Spatial Questions, Next Steps

```markdown
/\/\/\/\/\/\/\/\
```

- Runtime carrier coercion — ruled and implemented. Arithmetic reads masks and chars numerically and returns number; Greater/Lesser joins `mask < number < char`; comparison reads chars by code point, masks and numbers numerically, and symbols only under same-carrier `==` or `!=`. Registry declarations and effect destinations remain strict, so promotion grants no storage subtyping.

- Numeric exceptional values — ruled and implemented. `num` is finite IEEE float64 plus `±∞`; NaN and absence are not values, and either machine signed zero canonicalizes to the sole Ano zero `0`. Load, migration, save, and numeric publication enforce that boundary, with computed NaN refused before commit. Refined numeric carriers remain finite. Infinity does not become a numeric extrema identity, so an empty identityless fold still yields no result row.

- Explicit bindings — ruled and implemented. One name declares exactly one face: entity, mask, point, number, or vector. Context never changes that denotation. An entity binding contains a stable key resolved uniquely through the registered key role on every world snapshot, never a row offset; a mirror-read therefore survives row compaction without turning the binding into a surface handle.

- Recurrences — ruled and implemented at the admitted boundary. A true within-statement recurrence is `scan(f) col along order`: exact sequential left accumulation on the read side, including non-associative steps. The result carries its ordered world-row domain. Scatter inverse-grades values and witness together, refuses statically decidable selection mismatches, and asserts exact witness equality with the statement's selected rows before commit; equal length alone grants nothing, so duplicates, omissions, and foreign same-width domains refuse. Read and write footprints must also be disjoint, with self-recurrence taking diagnostic precedence. `prev` remains a parallel shift, not a carry, general fixpoint/iterate and implicit generator clauses remain outside the language, and recurrence across ticks remains the host's fold of the one-barrier transition.

- Identity — ruled. The surface stays intensional: no resolved-selection handle survives a statement or tick. Extensional identity lives only in world data—stable keys, relationship components, explicit marker components, and tick-stamped history. “The same bandit as last tick” is therefore a relationship/key read or, when history is registered, an as-of join against the t−1 partition; it is never a captured predicate result. The saved-mask continuation remains statement-local.

- Rule retraction — ruled and implemented. `undef name`, 日本語 `解除 name`, removes exactly one named installed rule at a barrier. Unknown and duplicate live names refuse; anonymous rules are world-lifetime. Retraction is explicit replay state, not scope expiry, guard withdrawal, or an effect a rule can race inside its own barrier.

- Staging and quotation — ruled closed at the literal splice. `eval "…"` accepts exactly one literal statement, re-lexes and parses it at compile time, and exposes its footprint to ordinary checks. Runtime strings, interpolation, templates with holes, rules that manufacture rules, macros, and a koto code value are rejected rather than deferred to an interpreter. A future typed code carrier would be a new proposal with typed free variables and effects, not an extension inferred from strings.

- Outer product as a value — ruled and implemented. `cross f A B` materializes the N×M result without flattening and carries product lineage distinct from the entity row domain. It may be observed as a rank-2 value, but no pure wrapper may make it scatter-compatible; assigning it to a world column refuses. The double-generator comprehension remains the relational form when pair rows, filters, and effects are wanted.
...
- The Sky Registry witness — ruled. Rewrite authority is binary: a law carries a machine-checked proof certificate for the exact typed operation, endpoints, and schema identity, or the optimizer may not use it. Trusted declarations and property tests may admit executable ground behavior and find defects, but they confer no theorem status. BQN twins remain explanatory witnesses, never semantic or differential oracles. `ano-sky.md` carries the certificate boundary.

- The machine as a world — ruled as a falsifiable research backend, not a core-language roadmap. The first gate is the Haskell embedding of the evaluator and accepted-plan semantics; only a semantics-preserving specialization result may justify a machine-world prototype. Dependence and layout are proof targets, latency is measured, literal self-modifying code remains rejected, and usable synthesis writes fresh code columns through the fixed bytecode/JIT boundary. Failure of the prototype changes no Ano denotation.
...
- Reap ownership and granularity — ruled world-wide. Generational tombstoning marks through the generation bump and presence clear; one shared entity-row topology and free list reclaims at tick seal or host command according to the registry's single `reap seal|host` policy. Per-archetype or per-column reaping would let aligned columns disagree about row identity and is rejected. The mask meaning of `~` never changes.
