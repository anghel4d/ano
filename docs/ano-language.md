...
A `Σ → Σ′` change is a host operation at a world barrier, never an Ano effect. Steel parses the candidate independently, requires a complete explicit declaration map, stages supported carrier conversions, seals relationship endpoints, migrates the live alias overlay, and mints a new `(version,fingerprint)`. The fingerprint covers every current declaration descriptor, spelling alias, role, and reap policy while deliberately excluding live row values and population. Stable declaration IDs survive `preserve`, `rename`, and supported `widen` rules; additions mint IDs; removals with live data require `discard`; aliases targeting a removed declaration require `unalias`. Every compiled plan and resolved declaration handle is schema-stamped and must be revalidated through the migration receipt or discarded. The pre-spatial implementation accepts only `bool → nat|int|num`, `nat → int|num`, and `int → num` widening; callable bodies and relationship endpoint identities must remain compatible because the author-gated signature taxonomy does not yet exist.

The persistent boundary is deliberately outside both grammars: `<world>.reg.schema` stores stable IDs and schema version, `<world>.reg.aliases` stores the session overlay, and `<world>.reg.migrations` records deterministic events. `kore migrate live.reg candidate.reg migration.map` publishes those three artifacts with the migrated `.reg` as one rollback-safe journaled transaction. The typed migration-extension trait is the only route by which task `99` may add lattice, habitat, placement, and spatial-service conversion.
