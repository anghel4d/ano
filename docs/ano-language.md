...
Ano's Greater/Lesser family is exactly q/kdb+'s convention over the carriers that Ano admits; this is not a claim of general q compatibility. `a | b` is OR on masks, pointwise maximum on numbers, and the pointwise greater rune on glyphs. `a & b` is AND on masks, pointwise minimum on numbers, and the pointwise lesser rune on glyphs. The glyph order is the code-point order. Their folds and scans are the corresponding reductions and scans. Steel currently admits same-carrier instances and refuses mixed carriers before lowering. Ano's expression-level coercion table is not yet ruled or implemented; registry declarations remain strict and every committed write is checked against its destination carrier. q's `98 | "a"` lifts the int into a char and yields `"b"`; current Steel refuses that expression. See KX's definitions of [Greater](https://code.kx.com/q/ref/greater/), [Lesser](https://code.kx.com/q/ref/lesser/), [max](https://code.kx.com/q/ref/max/), and [min](https://code.kx.com/q/ref/min/).
...
The overlay runtime, the resolver framework, the host boundary, replay, and Kore's display are implemented in Steel and Kore; `todo/02-dynamic-alias-overlay.md` still owes the ruled `^cursor` cold default at the elided-subject site, the concrete host deictic resolvers, and the alias demo rewrites.
...
- Runtime carrier coercion. Registry declarations remain strict, while runtime expressions may admit operator-specific coercions before a result is checked against its destination carrier. The exact operand-and-result table is open. q does not provide one universal hierarchy: Greater/Lesser and arithmetic can lift the same pair in different directions. Steel therefore refuses every mixed carrier before lowering until Ano has an explicit table rather than guessing one.
