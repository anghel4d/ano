...
anostr_t anostr_cull(ano_arena_t *heap, anostr_t s, uint32_t classes)
{
...
anostr_t anostr_rune_sort(ano_arena_t *heap, anostr_t s)
{
    if (s.len < 2)
        return s;
    const uint8_t *p = (const uint8_t *)anostr_bytes(&s);
    ...
    anorune_t *runes = malloc((size_t)s.len * sizeof *runes);
    if (runes == NULL)
        return anostr_empty();
    size_t n = 0;
...
anostr_t anostr_from_utf16(ano_arena_t *heap, const char16_t *src, size_t count)
{
    if (src == NULL)
        return anostr_empty();
...
anostr_t anostr_from_utf16_cstr(ano_arena_t *heap, const char16_t *src)
{
    if (src == NULL)
        return anostr_empty();
    size_t count = 0;
...
char16_t *anostr_to_utf16(ano_arena_t *heap, anostr_t s, size_t *count)
{
    if (count != NULL)
        *count = 0;
    if (heap == NULL)
    ...
    char16_t *out = ano_arena_alloc(heap, ((size_t)s.len + 1) * sizeof(char16_t));
    if (out == NULL)
        return NULL;
    size_t n = 0;
    for (size_t i = 0; i < s.len; ) {
    ...
    char16_t *exact = ano_arena_realloc(heap, out, (n + 1) * sizeof(char16_t));
    return exact != NULL ? exact : out;
}

anostr_t anostr_from_utf32(ano_arena_t *heap, const anorune_t *src, size_t count)
{
    if (src == NULL)
        return anostr_empty();
...
anorune_t *anostr_to_utf32(ano_arena_t *heap, anostr_t s, size_t *count)
{
    if (count != NULL)
        *count = 0;
    if (heap == NULL)
    ...
    anorune_t *out = ano_arena_alloc(heap, ((size_t)s.len + 1) * sizeof(anorune_t));
    if (out == NULL)
        return NULL;
    size_t n = 0;
    for (size_t i = 0; i < s.len; )
    ...
    if (count != NULL)
        *count = n;
    anorune_t *exact = ano_arena_realloc(heap, out, (n + 1) * sizeof(anorune_t));
    return exact != NULL ? exact : out;
}
