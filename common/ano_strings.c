...
// Allocation policy: nothing owns memory. Bytes live in a caller-supplied ano_arena_t, freed at heap teardown.
...
anostr_t anostr_from(ano_arena_t *heap, const void *bytes, size_t len)
{
    ...
    if (heap == NULL)
        return anostr_empty();
    char *copy = ano_arena_alloc(heap, len);
    if (copy == NULL)
        return anostr_empty();
...
anostr_t anostr_from_cstr(ano_arena_t *heap, const char *cstr)
{
    if (cstr == NULL)
        return anostr_empty();
...
anostr_t anostr_keep(ano_arena_t *heap, anostr_t s)
{
...
char *anostr_to_cstr(ano_arena_t *heap, anostr_t s)
{
    if (heap == NULL)
        return NULL;
    char *out = ano_arena_alloc(heap, (size_t)s.len + 1);
    if (out == NULL)
        return NULL;
...
anostr_builder_t anostr_builder_make(ano_arena_t *heap, uint32_t reserve)
{
    anostr_builder_t b = { .ptr = NULL, .len = 0, .cap = 0, .heap = heap };
    if (heap != NULL && reserve > 0) {
        b.ptr = ano_arena_alloc(heap, reserve);
        if (b.ptr != NULL)
            b.cap = reserve;
    }
    return b;
...
static int builder_reserve(anostr_builder_t *b, uint64_t need)
{
    ...
    if (cap > UINT32_MAX)
        cap = UINT32_MAX;
    char *grown = ano_arena_realloc(b->heap, b->ptr, cap);
    if (grown == NULL)
        return -1;
    b->ptr = grown;
    b->cap = (uint32_t)cap;
...
anostr_t anostr_freeze(anostr_builder_t *b)
{
    ...
    if (b->len <= ANOSTR_INLINE_CAP) {
        ...
        ano_arena_free(b->heap, b->ptr);    // inline value owes nothing to the buffer
    } else {
        // Shrink to exactly len and hand the buffer to the value. A failed shrink keeps the original block.
        char *exact = ano_arena_realloc(b->heap, b->ptr, b->len);
