...
static void kb_bytes(key_buf_t *kb, const void *src, size_t n)
{
    if (kb->n + n > kb->cap) {
        ...
        while (cap < kb->n + n)
            cap *= 2;
        uint8_t *fresh = realloc(kb->p, cap);
        if (fresh == NULL) {
            kb->oom = true;
            return;
        }
...
{
    tie_view_t *views = malloc(n * sizeof *views);
    if (views == NULL)
        return false;
...
static void tie_msd(sort_rec_t *r, size_t n, uint32_t skip, rec_str_fn_t str_of, const void *ctx)
{
    ...
    if (n <= 48) {
        ...
        sort_rec_t *tmp = malloc(n * sizeof *tmp);
        if (tmp == NULL) {
            tie_leaf(r, n, str_of, ctx);
            return;
        }
...
void anostr_sort(anostr_t *items, size_t count)
{
    if (items == NULL || count < 2)
        return;
    sort_rec_t *recs = count <= UINT32_MAX ? malloc(2 * count * sizeof *recs) : NULL;
...
void anostr_sort_idx(const anostr_t *items, size_t count, uint32_t *order)
{
    ...
    sort_rec_t *recs = malloc(2 * count * sizeof *recs);
    if (recs == NULL) {
        fb_items_ = items;
...
static const uint64_t *sym_key_cache(anostr_intern_t *t)
{
    ...
    if (t->collateKeyCap < t->count) {
        uint64_t *fresh = ano_arena_realloc(t->heap, t->collateKeys,
