...
//
// Threading: single mutator, same rule as the arena underneath. Readers (find/sym_str/
...
anostr_intern_t *anostr_intern_make(ano_arena_t *heap)
{
    if (heap == NULL)
        return NULL;
    anostr_intern_t *t = ano_arena_zalloc(heap, sizeof *t);
    if (t == NULL)
        return NULL;
    t->slots = ano_arena_zalloc(heap, INTERN_INITIAL_SLOTS * sizeof *t->slots);
    if (t->slots == NULL) {
        ano_arena_free(heap, t);
        return NULL;
    }
    t->heap = heap;
...
static int grow_arrays(anostr_intern_t *t)
{
    ...
    if (newCap > UINT32_MAX)
        return -1;
    uint64_t *hashes = ano_arena_realloc(t->heap, t->hashes, (size_t)newCap * sizeof *hashes);
    if (hashes == NULL)
        return -1;
    t->hashes = hashes;    // committed independently: a failed strs growth still leaves
    anostr_t *strs = ano_arena_realloc(t->heap, t->strs, (size_t)newCap * sizeof *strs);
    if (strs == NULL)
