...
// heap must be owned by the calling thread (arenas are single-owner). 
...
anostr_t anostr_concat(ano_arena_t *heap, anostr_t a, anostr_t b);
...
anostr_t anostr_join(ano_arena_t *heap, anostr_t sep, const anostr_t *parts, size_t count);
...
anostr_intern_t *anostr_intern_make(ano_arena_t *heap);
...
typedef struct anostr_builder_t {
    ...
    uint32_t   len;
    uint32_t   cap;
    ano_arena_t *heap;  // NULL marks a consumed/discarded builder: appends fail with -1
} anostr_builder_t;
