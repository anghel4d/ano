...
kore draws EVERY positioned entity as a hardcoded bold `@` (kore.c:2481), stamped over the field layer. The 6-space demos are data-sound — every expect passes, both suites green — but the renderer is glyph-blind for entities: 27-lattice-patterns (an entity on every cell) is a solid wall of @; 34-board-literal is 32 @'s while its `proto` column, which holds the actual piece glyphs, is ignored. "Broken, just shows a grid of @'s" was the renderer, not the demos.

## Work items

- Square cells: a TUI cannot load fonts; render each map cell two columns wide instead, giving ~1:1 cells in any font (and trivial mouse mapping). A genuinely square font is a terminal-profile matter, or a later standalone shell drawn from the anoptic-engine parts bin.
