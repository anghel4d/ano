...
One decision blocks this task's close: whether Ano adopts implicit carrier coercion in runtime expressions, and under what table. Every carrier in this file currently refuses a mixed operand before lowering, and `98 | "a"` is pinned as a refusal with an exact diagnostic. A9 borrows q's Greater/Lesser convention and expressly disclaims general q compatibility, so coercion is not derivable from it and is a decision in its own right. q's own model is operator-specific rather than one hierarchy: Greater and Lesser return the greater or lesser underlying value as the higher of the two types, so q's `98 | "a"` is the char `"b"`, while q's arithmetic lifts a char the other way, to an int. Following q therefore means an operand-and-result-carrier table per operator family, not a blanket conversion rule. The separable sub-questions: which mixed pairs are legal and the result carrier of each; whether coercion is per-operator or universal; whether the mask carrier participates in arithmetic, which would let `+/` over a stored bool column agree numerically with `#/` over the same rows — a redundancy q accepts deliberately, and a policy choice about how strictly stored bools, predicate masks, and numbers stay separated, not a structural hazard; and how a coerced result is checked when committed, where the stated design position is that registry declarations stay strict and only expression-level carriers flex, so every write is still checked against the declared destination carrier. Independent of coercion entirely: none, NaN, and the infinities are three separate admissions that A12 currently closes together, and admitting any one would be a new ruling superseding A12, not a consequence derivable here. Decide before the demo corpus is rewritten against the present behavior.
...
The char instances follow from A9 by derivation; they are not a separate ruling. The order is the code-point order, so `"sat" | "cow"` is `"sow"`, `"sat" & "cow"` is `"cat"`, `max/ "genie"` is `'n'` and `min/ "genie"` is `'e'`. There is no coercion between any two carriers and no `||`. Mixed carriers refuse before lowering; q itself lifts the mixed case to the higher of the two types, so its `98 | "a"` is the char `"b"`, and Ano declines exactly that coercion — `98 | "a"` is a q expression and not an Ano one — and char against mask refuses for the same reason. A9 adopts q's convention over Ano's carriers, never q's type-promotion rules.
