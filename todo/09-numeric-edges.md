...
Surfaced by the 2026-07-10 overflow experiments (live, verified); no ruling requested then, filed now. Two halves: document the number model, and close a self-sealing gap in it.
...
A hand-written `inf` in a .reg LOADS (strtod parses it to INFINITY) and then dies downstream: numLit spells it `inf`, BQN sees an undefined identifier, the program crashes with no useful diagnostic. So the system refuses to WRITE a non-finite world but accepts READING one — an asymmetry: anything anoc saves it can load, but not everything it loads could have been saved.

## Work items

1. Rule and implement the closure — recommendation from review: refuse non-finite values (inf, -inf, nan, and any strtod spelling of them) at registry LOAD, mirroring the save's refusal, with a line/column diagnostic. The invariant becomes airtight: the registry's value domain is exactly the finite doubles; load ∘ save = identity on it. The alternative (teach numLit BQN's ∞ literal and let worlds go non-finite) trades the determinism stance for nothing the demos want; surface it, but the refusal is the coherent seal.
