...
1. [X] DONE — **Preserve sigil semantics through resolution.** `01-sigil-semantics.md` keeps `!` as mask negation, keeps `^name` distinct from bare `name` through the resolved IR, and separates spelling aliases, static alias masks, and dynamic aliases.
