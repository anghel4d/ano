...
5. **Finish non-spatial registry/runtime follow-ups.** `05-registry-and-runtime-followups.md` now supplies the atomic, journaled pre-spatial `Σ → Σ′` framework, schema-stamped handles, complete migration maps, typed task-`99` extension boundary, and bounded OSC 10/11 palette negotiation while keeping registry and Ano grammar separate. The general registry taxonomy remains intentionally blocked on explicit approval of its exact syntax.
