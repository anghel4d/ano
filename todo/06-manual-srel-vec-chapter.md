...
- Vectors as cell values exist in the disciplined form: `col pos vec` holds fixed-arity pairs; `bind pathCells vec` holds index sequences. The discipline, stated: fixed-shape tuples live in vec columns; ragged, variable-arity data lives behind a relation — named, with fibers, an inverse, and the γ machinery. Both are array-native; neither leaks raggedness into the mask algebra.
