...
Ruled (author, 2026-07-11): the permutation table goes into the documentation or the grammar, verbatim; `&\` and `|\` "most definitely do deserve demos"; a full table showing everything together — all of `+/ */ …` AND `+\ *\ …` in one place.
...
The identity column is the §12/§13 law restated: a fold with a registered identity yields it on the empty scope; a reducer without one fails the row — left-join-null extended to the empty fiber. The γ column-form (`f/ rel'.Comp`) inherits the same identities per fiber.
