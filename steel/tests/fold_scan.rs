...
// Greater/Lesser uses its representation join: mask < number < char. This is deliberately
// distinct from arithmetic promotion, which always lands at number.
#[test]
fn mixed_carriers_promote_by_operator() {
    for source in ["98 | \"a\"\n", "Rune | Gold\n", "Rune & Burning\n"] {
        let text = ok(source);
        assert!(text.contains("Fn_AnoSemCodeChar"), "{}: {}", source.trim_end(), text);
        assert!(
            text.contains("Fn_AnoSemCharGreater") || text.contains("Fn_AnoSemCharLesser"),
            "{}: {}",
            source.trim_end(),
            text
        );
    }
    let numeric = ok("Silver = (Gold | Burning)\n");
    assert!(numeric.contains("Fn_AnoSemMaskNumber"), "{}", numeric);
    assert!(numeric.contains("Fn_AnoSemGreater"), "{}", numeric);
    assert!(ok("Burning , Rune = (Gold | Rune)\n").contains("Fn_AnoSemCharGreater"));
    assert_eq!(
        err("Burning , Silver = (Gold | Rune)\n"),
        "emit: line 1: cannot write char result to number 'Silver'"
    );
    assert_eq!(
        err("Burning , Gold = :Red\n"),
        "emit: line 1: cannot write sym result to number 'Gold'"
    );
    // a predicate is selection, not Lesser: the mask reading survives untouched
    assert!(ok("Rune & Burning , +Path\n").contains("burning"));
}

// Arithmetic and its fold/scan forms consume code points; registered reducers stay nominal.
#[test]
fn arithmetic_reducers_promote_char_codes() {
    for source in [
        "+/ Rune\n",
        "*/ Rune\n",
        "-/ Rune\n",
        ...
        let text = ok(source);
        assert!(text.contains("Fn_AnoSemCharCode"), "{}: {}", source.trim_end(), text);
    }
    assert_eq!(
        err("threat/ Rune\n"),
        "emit: line 1: reducer 'threat' is not defined on Char"
    );
...
#[test]
fn carrier_gate_and_promotion_are_uniform_across_forms() {
    let heads = [("max", "max"), ("min", "min"), ("threat", "threat")];
    ...
    for source in [
        ...
        "avg/ Burning\n",
        "avg\\ Burning\n",
        "scan(avg) Burning along Silver\n",
    ] {
        let text = ok(source);
        assert!(
            text.contains("Fn_AnoSemMaskNumber"),
            "{}: {}",
            source.trim_end(),
            text
        );
...
#[test]
fn scan_scatter_requires_disjoint_read_and_write_footprints() {
    assert_eq!(
        err("Burning , Gold = +\\ Gold\n"),
        "emit: line 1: scan reads and writes 'Gold'; recurrence footprints must be disjoint"
    );
    assert_eq!(
        err("Burning , Gold = scan(+) Silver along Gold\n"),
        "emit: line 1: scan reads and writes 'Gold'; recurrence footprints must be disjoint"
    );
    let disjoint = ok("Burning , Silver = +\\ Gold @ Burning\n");
    assert!(disjoint.contains("silver ↩"), "{}", disjoint);
}

#[test]
fn numeric_publication_seals_computed_nan_before_commit() {
    let emitted = ok("Burning , Gold = Gold - Gold\n");
    let seal = emitted
        .find("\"numeric Gold\" !")
        .expect("numeric carrier seal");
    let publish = emitted.find("gold ↩").expect("numeric publication");
    assert!(seal < publish, "{emitted}");
    assert!(emitted[seal..publish].contains("=("), "{emitted}");
}
...
// Greater/Lesser joins mask and number at number in value position. A selection remains a mask.
#[test]
fn mixed_direct_carriers_promote() {
    let greater = ok("Silver = (Gold | Burning)\n");
    assert!(greater.contains("Fn_AnoSemMaskNumber"), "{}", greater);
    assert!(greater.contains("Fn_AnoSemGreater"), "{}", greater);
    let lesser = ok("Silver = (Gold & Burning)\n");
...
// Heads with no declared promotion refuse identically on both surfaces.
#[test]
...
// A keyed set relationship is key-space data even before the first structural barrier. A
...
// in the plan; an admitted plan still carries an extensional witness to the barrier.
#[test]
fn scan_scatter_seals_its_exact_row_domain() {
    assert_eq!(
        err("Path , Gold = +\\ Silver @ Burning\n"),
        "emit: line 1: scan domain does not match statement selection for 'Gold'"
    );
    assert_eq!(
        err("Path , Gold = +\\ Silver\n"),
        "emit: line 1: scan domain does not match statement selection for 'Gold'"
    );

    let aligned = ok("Path , Gold = +\\ Silver @ Path\n");
    assert!(aligned.contains("\"scan domain Gold\" !"), "{}", aligned);
    assert!(aligned.contains("(path/↕anoN)"), "{}", aligned);
    assert!(aligned.contains("(s1m/↕anoN)"), "{}", aligned);
}
